...
6 Prisma models: `Domain` → `Subdomain`, `DomainRequest` → `Approval`, `Comment`, `AuditLog`

Every write operation should create an `AuditLog` entry.
...
- Dockerfile runs `prisma migrate deploy` on startup before serving
